...
def run_all_tests(tests, timeout=60):
    ...
    for f, args, name, group in tests:
        test_result = _run_test(f, timeout, args, name)
        
        overall_score += test_result.score
        overall_max += test_result.max_score
        group_scores[group] = group_scores.get(group, 0) + test_result.score
        group_maxes[group] = group_maxes.get(group, 0) + test_result.max_score
        results.append(test_result)

    return results, overall_score, overall_max, group_scores, group_maxes
